...
~/PrivoxKeys/AuthKey_723D52BFNL.p8   ключ APNs для iOS-push (копия на Рабочем столе)
```

**Ключ APNs** (`.p8`, выпущен 24.08.2026) — такой же невосстановимый секрет, как
keystore: Apple отдаёт файл при создании ровно один раз, потерян — только выпуск
нового взамен. Ключи APNs при этом **не истекают**, в отличие от сертификатов.

    Key ID   723D52BFNL   (он же в имени файла)
    Team ID  2ND5G88WGD
    Bundle   tech.privox.ptt

Key ID и Team ID секретом не являются — секрет только сам файл.
