...
`capacitor.config.json`), значит и звонок ему шлёт прод.

**✅ Прод уже настроен — 24.08.2026, переделывать не нужно.** Пять переменных
`APNS_*` добавлены в Coolify, ключ передан содержимым (в контейнере файлов нет,
секреты приходят переменными — как `FIREBASE_SERVICE_ACCOUNT_JSON` рядом).

Проверить одной командой в любой момент:

    curl -s https://ptt.privox.tech/health | grep -o '"apns":{[^}]*}'

Ожидается `{"configured":true,"env":"sandbox"}`. `configured: false` означает,
что прод ключа не видит — тогда смотреть переменные в Coolify, а нативную часть
подозревать не надо.

⚠️ **`env` менять при переходе на TestFlight.** Сейчас `sandbox` — верно для
сборок из Xcode. TestFlight и App Store ходят в `production`; адреса у Apple
разные, push «не туда» молча не доходит.
