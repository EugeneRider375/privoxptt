 ...
 *
 * Сделано печатью, потому что реально с этим делают именно это: печатают,
 * разрезают и раздают людям на руки.
 *
 * Здесь раньше стояло, что «диалог печати умеет Сохранить как PDF, так что
 * отдельный формат не нужен». Оказалось неверно: у Eugene при сохранении в файл
 * QR-коды оставались, а текст пропадал (на бумагу при этом печаталось нормально).
 * Воспроизвести в headless Chrome не удалось — там текст на месте, значит дело в
 * конкретном браузере. Поэтому появился `invitePdf.ts`: он собирает файл кодом и
 * диалог печати не задействует вовсе.
 *
